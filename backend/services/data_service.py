def get_segments_for_quiz(db, lecture_id):
    ...
    if not doc or "lectures" not in doc or not doc["lectures"]:
        ...
        try:
            ...
            if sample and "lectures" in sample:
                print(f"💡 Try testing with this ID instead: {sample['lectures'][0].get('lectureId')}")
        except:
            pass
    ...
    try:
        ...
        if not segments:
            print(f"⚠️ Lecture found but 'segments' array is empty for: {lecture_id}")
        ...
        print(f"✅ Successfully generated context for {len(segments)} segments.")
        return context

    except Exception as e:
        print(f"❌ Error parsing lecture segments for {lecture_id}: {str(e)}")
        import traceback
        traceback.print_exc()
